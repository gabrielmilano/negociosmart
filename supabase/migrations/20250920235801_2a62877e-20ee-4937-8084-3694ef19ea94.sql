-- Sistema multi-tenant com ambientes e tokens - Versão corrigida
...
-- Enable RLS apenas se as tabelas não tiverem RLS habilitado
DO $$
BEGIN
  IF NOT (SELECT row_security FROM pg_class WHERE relname = 'ambientes_empresa') THEN
    ALTER TABLE public.ambientes_empresa ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT (SELECT row_security FROM pg_class WHERE relname = 'tokens_ambiente') THEN
    ALTER TABLE public.tokens_ambiente ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF NOT (SELECT row_security FROM pg_class WHERE relname = 'relacionamentos_empresa') THEN
...
DROP POLICY IF EXISTS "Users can view environments from their companies" ON public.ambientes_empresa;
DROP POLICY IF EXISTS "Users can insert environments in their companies" ON public.ambientes_empresa;
DROP POLICY IF EXISTS "Users can update environments in their companies" ON public.ambientes_empresa;
DROP POLICY IF EXISTS "Users can view tokens from their company environments" ON public.tokens_ambiente;
DROP POLICY IF EXISTS "Users can insert tokens in their company environments" ON public.tokens_ambiente;
DROP POLICY IF EXISTS "Users can update tokens in their company environments" ON public.tokens_ambiente;
DROP POLICY IF EXISTS "Fornecedores veem seus relacionamentos" ON public.relacionamentos_empresa;
DROP POLICY IF EXISTS "Lojas veem seus relacionamentos" ON public.relacionamentos_empresa;
DROP POLICY IF EXISTS "Lojas podem solicitar relacionamento" ON public.relacionamentos_empresa;
DROP POLICY IF EXISTS "Fornecedores podem atualizar relacionamentos" ON public.relacionamentos_empresa;
DROP POLICY IF EXISTS "Lojas veem produtos de fornecedores relacionados" ON public.produtos;
...
-- Criar ambientes padrão para empresas existentes apenas se não existirem
...
-- Funções e triggers apenas se não existirem
...
DROP TRIGGER IF EXISTS trigger_auto_generate_token ON public.tokens_ambiente;
...
DROP TRIGGER IF EXISTS update_ambientes_empresa_updated_at ON public.ambientes_empresa;
