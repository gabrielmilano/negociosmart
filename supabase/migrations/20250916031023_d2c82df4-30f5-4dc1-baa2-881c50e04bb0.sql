-- Create triggers to ensure default user profile and company are created on signup
BEGIN;

-- 1) Attach trigger to create default company and membership for new auth users
...
-- 2) Attach trigger to create usuarios profile for new auth users
...
-- 3) Backfill missing usuarios for existing auth users
...
-- 4) Backfill default companies and memberships for users that don't have one
...
inserted_empresas AS (
  INSERT INTO public.empresas (
    nome, cnpj, email, telefone, endereco, usuario_proprietario_id, plano, status, configuracoes
  )
